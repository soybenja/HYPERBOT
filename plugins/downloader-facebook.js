...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  if (!args[0]) throw `🔮 𝙄𝙣𝙜𝙧𝙚𝙨𝙚 𝙨𝙪 𝙚𝙣𝙡𝙖𝙘𝙚 𝙙𝙚 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠 \n𝙀𝙟𝙚𝙢𝙥𝙡𝙤: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`;
  if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `🔮 𝙄𝙣𝙜𝙧𝙚𝙨𝙚 𝙪𝙣 𝙚𝙣𝙡𝙖𝙘𝙚 𝙙𝙚 𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠 𝙀𝙟𝙚𝙢𝙥𝙡𝙤: ${usedPrefix + command} https://fb.watch/fOTpgn6UFQ/`;
  try {
    await m.reply(`🔮 𝘾𝙖𝙧𝙜𝙖𝙣𝙙𝙤..... 𝙚𝙨𝙥𝙚𝙧𝙚 𝙪𝙣 𝙢𝙤𝙢𝙚𝙣𝙩𝙤 𝙥𝙤𝙧𝙛𝙖𝙫𝙤𝙧`);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          await conn.sendFile(m.chat, urll, 'error.mp4', '🔮 𝘼𝙦𝙪𝙞 𝙩𝙞𝙚𝙣𝙚 𝙨𝙪 𝙫𝙞𝙙𝙚𝙤', m);
        } catch (err3) {
          try {
            const res = await fbDownloader(args[0]);
            for (const result of res.download) {
              const ur = result.url;
              await conn.sendFile(m.chat, ur, 'error.mp4', '🔮 𝘼𝙦𝙪𝙞 𝙚𝙨𝙩𝙖 𝙨𝙪 𝙫𝙞𝙙𝙚𝙤', m);
            }
          } catch (err4) {
            try {
