...
export async function before(m, {isAdmin, isBotAdmin, isOwner}) {
  ...
  if (isToxic && chat.antiToxic && !isOwner && !isAdmin) {
    user.warn += 1;
    if (!(user.warn >= 5)) await m.reply('*[❗] ' + `${user.warn == 1 ? `❗𝘽𝙊𝙉𝙄𝙏𝙊 𝘿𝙄𝘼 @${m.sender.split`@`[0]}` : `@${m.sender.split`@`[0]}`}, 𝙙𝙚𝙘𝙞𝙧 𝙡𝙖 𝙥𝙖𝙡𝙖𝙗𝙧𝙖 "${isToxic}" 𝙚𝙨𝙩𝙖 𝙥𝙧𝙤𝙝𝙞𝙗𝙞𝙙𝙤 𝙚𝙣 𝙚𝙨𝙩𝙚 𝙜𝙧𝙪𝙥𝙤. 𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼: ${user.warn}/5.` + '*', false, {mentions: [m.sender]});
  }
